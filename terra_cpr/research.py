...
def generate_point_in_time_events(
    ...
    """Generate completed-close candidate activations without future leakage.

    Universe membership, feature history, beta/RS inputs, and daily structure are
    rebuilt at every timestamp. Outcomes are attached only after the complete
    event set has been generated by the frozen scanner rule.
    """
    if horizon_bars <= 0:
    ...
    for benchmark_index in range(0, max(0, len(benchmark_bars) - horizon_bars)):
        ...
        for row in rows:
            if (
                ...
                continue
            state = (row.setup.label, row.setup.direction)
            ...
            pending.append((
                as_of, row.symbol, row.setup.direction, row.price,
                float(row.rs.score), features,
            ))
